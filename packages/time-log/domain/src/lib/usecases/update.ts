...
@Injectable()
export class UpdateTimeLogUseCase implements Usecase<UpdateTimeLogDto, ResponseTimeLogDto> {
    constructor(
    ...
    async execute(input: UpdateTimeLogDto): Promise<ResponseTimeLogDto> {
        const findedTimeLog = await this.findTimeLogByIdUseCase.execute(input.id as keyof TimeLog);

        const timeLogDomain = this.timeLogMapper.toEntity(input);

        const updatedTimeLog = await this.timeLogRepository.updateOne(findedTimeLog.id as keyof TimeLog,
            {
                ...findedTimeLog,
                ...timeLogDomain
            }
